...
export default function Page() {
  ...
  const [openUpdateAgentModal, setOpenUpdateAgentModal] = useState(false)
  ...
  return (
    <>
        ...
        <Table.Body>
          {agents.map((agent) => (
            <Table.Row key={agent?._id}>
              ...
              <Table.Cell>{agent?.user?.email}</Table.Cell>
              <Table.Cell>{agent?.capacity}</Table.Cell>
              <Table.Cell>{agent?.level}</Table.Cell>
              <Table.Cell>
                <Flex gap='2'>
                  <IconButton
      ...
      <Dialog.Root open={openUpdateAgentModal} onOpenChange={setOpenUpdateAgentModal}>
        <UpdateAgentModal
          currentAgent={selectedAgent}
          onSuccess={() => {
            loadAgents()
            setOpenUpdateAgentModal(false)
          }}
        />
      </Dialog.Root>
    </>
